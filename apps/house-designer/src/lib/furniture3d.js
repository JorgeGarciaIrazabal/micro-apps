...
export function buildFurniture3D(type, w, d, h, color) {
  const mat = (c, { rough = 0.82, metal = 0.05, opacity = 1 } = {}) =>
  ...
  const box = (bw, bh, bd, x, y, z, c, opts) => {
    const m = new THREE.Mesh(new THREE.BoxGeometry(bw, bh, bd), mat(c, opts))
    m.position.set(x, y, z); g.add(m); return m
  }
  const cyl = (r, ch, x, y, z, c, opts) => {
    const m = new THREE.Mesh(new THREE.CylinderGeometry(r, r, ch, 20), mat(c, opts))
    m.position.set(x, y, z); g.add(m); return m
  }
  const sph = (r, x, y, z, c, opts) => {
    const m = new THREE.Mesh(new THREE.SphereGeometry(r, 18, 14), mat(c, opts))
    m.position.set(x, y, z); g.add(m); return m
  }
  ...
  switch (type) {
    case 'sofa':
    case 'armchair': {
      box(w, h * 0.4, d * 0.85, 0, h * 0.2, d * 0.05, color)
      box(w, h * 0.6, d * 0.22, 0, h * 0.7, -d / 2 + d * 0.11, dark)
      box(w * 0.1, h * 0.55, d * 0.85, -(w / 2 - w * 0.05), h * 0.275, d * 0.05, shade(color, 0.85))
      box(w * 0.1, h * 0.55, d * 0.85, (w / 2 - w * 0.05), h * 0.275, d * 0.05, shade(color, 0.85))
      box(w * 0.84, h * 0.12, d * 0.66, 0, h * 0.46, d * 0.08, light)
      break
    }
    case 'coffee-table':
    case 'dining-table':
    case 'desk': {
      box(w, h * 0.08, d, 0, h - h * 0.04, 0, color)
      for (const sx of [-1, 1]) for (const sz of [-1, 1])
        box(w * 0.08, h * 0.92, d * 0.08, sx * (w / 2 - w * 0.06), h * 0.46, sz * (d / 2 - d * 0.06), dark)
      break
    }
    case 'chair': {
    ...
    case 'office-chair': {
      ...
      cyl(w * 0.05, h * 0.45, 0, h * 0.27, 0, dark)
    ...
    case 'bed-single': {
      ...
      box(w, h * 0.3, d, 0, h * 0.15, 0, shade(color, 0.6))           // frame
      box(w * 0.96, h * 0.45, d * 0.94, 0, h * 0.45, 0, color)        // mattress
      box(w, h * 1.5, d * 0.1, 0, h * 0.75, -d / 2 + d * 0.05, shade(color, 0.5)) // headboard
      const py = h * 0.675 + h * 0.1
      for (let i = 0; i < pillows; i++) {
        const x = pillows === 1 ? 0 : (i === 0 ? -w * 0.22 : w * 0.22)
        box(pw, h * 0.2, d * 0.18, x, py, -d / 2 + d * 0.16, light)
      }
      break
    }
    case 'nightstand':
    ...
    case 'vanity': {
      ...
      box(w * 0.7, h * 0.12, d * 0.6, 0, h * 0.92, 0, light)         // basin
      box(w * 0.05, h * 0.22, d * 0.05, 0, h * 1.02, -d / 2 + d * 0.12, dark) // faucet
      break
    }
    case 'stove': {
      box(w, h, d, 0, h / 2, 0, color)
    ...
    case 'fridge': {
      ...
      box(w * 0.04, h * 0.5, d * 0.04, w / 2, h * 0.5, 0, dark)      // handle
      break
    }
    case 'toilet': {
      box(w * 0.9, h * 0.5, d * 0.3, 0, h * 0.75, -d / 2 + d * 0.15, color) // tank
      cyl(w * 0.42, h * 0.45, 0, h * 0.225, d * 0.12, light)                // bowl
      break
    }
    case 'bathtub': {
      box(w, h, d, 0, h / 2, 0, color)
    ...
    case 'rug': {
      box(w, Math.max(0.02, h), d, 0, Math.max(0.02, h) / 2, 0, color)
      break
    }
    default: {
      box(w, h, d, 0, h / 2, 0, color)
